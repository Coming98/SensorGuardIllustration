...
export async function load({ fetch }) {
    ...
    const res_item = await fetch(`/${item_name}.csv`);
    if (!res_item.ok) throw new Error(`Failed to load ${item_name}.csv`);
    ...
    if (!res_S9_json.ok) throw new Error(`Failed to load ${item_name}.json`);
